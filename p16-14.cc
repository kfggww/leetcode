...
class Solution {
  ...
  private:
        ...
        for (int after_index = 0; after_index < this->points.size();
            ...
            if (before_index != after_index && marked[after_index] != 1) {
                ...
                if (add_point_to(points[before_index], before_index,
                                 points[after_index], after_index, dx, dy,
                                 line)) {
    ...
    // 加入一个点到当前直线
    bool add_point_to(vector<int> &point_before, int i,
                      vector<int> &point_after, int j, int &dx, int &dy,
                      struct Line &line) {
        ...
        for (int i = 0; i < SIZE; ++i) {
            for (int j = i + 1; j < SIZE && SIZE >= count - 1 + j; ++j) {
                ...
                for (int k = j + 1; k < SIZE; ++k) {
                    if (dx * (points[k][1] - points[j][1]) ==
                        dy * (points[k][0] - points[j][0]))
                        tmp_count++;
                }
