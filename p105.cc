...
public:
  ...
  TreeNode *build(int *preOrder, int s1, int e1, int *inOrder, int s2, int e2) {
    if (s1 >= e1)
      return nullptr;
    ...
    // 构造左子树
    root->left =
        build(preOrder, s1 + 1, s1 + 1 + rootPos - s2, inOrder, s2, rootPos);
    // 构造右子树
    root->right =
        build(preOrder, e1 - e2 + rootPos + 1, e1, inOrder, rootPos + 1, e2);
    return root;
  }
